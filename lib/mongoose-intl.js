...
module.exports = exports = function mongooseIntl(schema, options) {
    ...
    // allow to use the fallback language in virtual getter, set to false if omitted
    if (typeof options.fallbackLanguage === 'string' && pluginOptions.languages.indexOf(options.fallbackLanguage) !== -1) {
        pluginOptions.fallbackLanguage = options.fallbackLanguage;
    } else {
        pluginOptions.fallbackLanguage = null;
    }
    ...
    schema.eachPath(function (path, schemaType) {
        ...
        var useFallbacks = typeof schemaType.options.useFallbacks === 'undefined'
          || schemaType.options.useFallbacks;
        ...
        schema.virtual(path)
            .get(function () {
                ...
                if (useFallbacks && pluginOptions.fallbackLanguage && langSubDoc.hasOwnProperty(pluginOptions.fallbackLanguage)) {
                    return langSubDoc[pluginOptions.fallbackLanguage];
                }
        ...
        var childOptions = extend({}, schemaType.options);
        delete childOptions.intl;

        var intlObject = {};
        intlObject[key] = {};
        pluginOptions.languages.forEach(function (lang) {
            var langOptions = extend({}, childOptions);
